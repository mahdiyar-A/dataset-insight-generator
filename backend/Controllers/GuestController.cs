...
public class GuestController : ControllerBase
{
    ...
    // Rate limited per client IP. sessionId is generated by the browser and
    // never verified, so it cannot bound anything — a caller mints a new GUID
    // per request.
    [HttpPost("upload")]
    [EnableRateLimiting("guest-upload")]
    ...
    // ── POST /api/guest/chat ──────────────────────────────────────────────
    // The expensive one: "start_analysis" and "yes" both trigger a full pipeline
    // run — Groq classification plus Gemini insight generation — with no account
    // behind it. This was completely unmetered.
    [HttpPost("chat")]
    [EnableRateLimiting("guest-analysis")]
