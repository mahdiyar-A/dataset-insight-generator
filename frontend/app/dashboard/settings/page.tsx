...
const SETTINGS = [
  ...
  },
  {
    ...
    icon: "🌐",
    items: [
      { id: "language", label: "Language",      desc: "Interface language.",                type: "select", options: ["English", "French", "Arabic"], tba: false },
...
export default function SettingsPage() {
  ...
  // Map locale code to display option string
  const LOCALE_TO_OPTION = { en: "English", fr: "French" };
  const OPTION_TO_LOCALE = { English: "en", French: "fr" };
  ...
  const select = (id, v) => {
    setValues(p => ({ ...p, [id]: v }));
    if (id === "language" && OPTION_TO_LOCALE[v]) {
      setLocale(OPTION_TO_LOCALE[v]);
    }
  };
              ...
              <div key={item.id} style={{
                ...
                {item.type === "select" && (
                  <select
                    ...
                    style={item.tba ? disabledSelectStyle : activeSelectStyle}
                    title={item.tba ? t("comingSoon") : undefined}
                  >
...
const activeSelectStyle   = { background: "rgba(15,23,42,0.95)", border: "1px solid rgba(37,99,235,0.5)", borderRadius: "8px", padding: "7px 12px", fontSize: "0.82rem", color: "#e5e7eb", cursor: "pointer", flexShrink: 0 };