...
export default function ProfileViewPage() {
  ...
  if (!currentUser) return <div style={loadingStyle}>{t("notAuthenticated")}</div>;
  ...
  const rows = [
    { label: t("rowFirstName"),   value: firstName },
    { label: t("rowLastName"),    value: lastName },
    { label: t("rowDisplayName"), value: currentUser.userName || "—" },
    { label: t("rowPhone"),       value: currentUser.phoneNumber || "—" },
    { label: t("rowEmail"),       value: currentUser.email || "—" },
    { label: t("rowMemberSince"), value: memberSince || "—" },
  ];

  return (
    <div style={pageStyle}>
      <div style={{ padding: "16px 24px 0" }}>
        <a href="/dashboard" style={ghostLink}>{t("backToDashboard")}</a>
      </div>
        ...
        <section style={cardStyle}>
          ...
          <div style={headerRowStyle}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
              ...
              {memberSince && (
                <p style={{ margin: "4px 0 0", color: "#4b5563", fontSize: "0.75rem" }}>{t("memberSince", { date: memberSince })}</p>
              )}
